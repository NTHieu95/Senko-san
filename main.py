...
pixiv_login = os.environ['PIXIV_EMAIL']
pixiv_password = os.environ['PIXIV_PW']
...
async def on_member_join(member):
    ...
    for channel in member.guild.channels:
        if str(channel.name) == "introduction": # We check to make sure we are sending the message in the general channel
            
            await channel.send('Xin chào '+ member.mention + ' đến với Ổ Dịch nanoja <:HinataYeah:597953425149394950>! Hãy **giới thiệu bản thân** và đọc '+  client.get_channel(595127051418206218).mention +' để hiểu rõ hơn về server này nhé nanoja <:HinataYeah:597953425149394950>!')

@client.event
async def on_member_remove(member):
    for channel in member.guild.channels:
        if str(channel.name) == "general-talk": # We check to make sure we are sending the message in the general channel
            
            await channel.send('Tạm biệt *'+ member.mention + '*(' + str(member) +')' +'<:Aqua_cry:595633225637036043>')
...
async def on_message(message):
    ...
    if(message.content != None):
        if(message.content.lower() == "xin lỗi senko" and str(user.id) in text):
    ...
    if str(user.id) in text:
        ...
        or (message.content.lower().find("địt") >= 0 and message.content.lower().find("senko") >= 0) \
        ...
        if (message.content.lower().find(" sad") >= 0 or message.content.lower().find("alexa") >= 0 or message.content.lower().find("buồn") == 0):
        ...
        elif message.content.lower().find("vả ") == 0 and message.content.lower().find("quạt") < 0 and message.content.lower().find("ranh") < 0 and message.content.lower().find("quat") < 0 and message.content.lower().find("rảnh") < 0:
            if message.content.lower().find("<@!") >= 0:
                slap(client.get_user(int(userFilterNick(message.content)[0])).name)
            elif message.content.lower().find("<@") >= 0:
                slap(client.get_user(int(userFilterOrig(message.content)[0])).name)
            else:
                slap(message.content[3:])
            await message.channel.send(file=discord.File('slapout.jpg'))
        elif message.content.lower().find("pat ") == 0:
            if message.content.lower().find("<@!") >= 0:
                pat(client.get_user(int(userFilterNick(message.content)[0])).name)
            elif message.content.lower().find("<@") >= 0:
                pat(client.get_user(int(userFilterOrig(message.content)[0])).name)
            else:
                pat(message.content[4:]) 
            await message.channel.send(file=discord.File('patout.jpg'))
        
        if message.content.lower().find(".gg ") == 0 and len(message.content) >3:
        ...
        if message.content.lower() == 'nanoja':
            embed = discord.Embed(
                title = 'Tiểu sử',
                description = 'Ta vốn là một chatbot trên facebook được Rảnh tạo ra vào một ngày đẹp trời tháng 5 năm 2019. Nhưng từ khi thanh niên Zucc nhúng tay vào thì ta được Rảnh cho chuyển sinh qua đây và hóa thành Senko vào 18/08/2019. \n今は童は仙狐じゃ！',
                colour = discord.Colour.orange()
            )
            # embed.set_image(url='https://tenor.com/6NXd.gif')
            embed.set_thumbnail(url='https://danbooru.donmai.us/data/__senko_sewayaki_kitsune_no_senko_san_drawn_by_araki495__c732565f9449561200981f705c540a78.png')
            embed.set_author(name='Senko của Rảnh nanoja!', icon_url='https://cdn.discordapp.com/emojis/595472994730770435.png')
            embed.add_field(name='Actions', value='vả, pat, chọn, .gg, .weather, .img, cưới, ly dị', inline=True)
            embed.add_field(name='Đừng buồn', value='Vì ta sẽ luôn ở bên để bật Despacito cho Người nghe', inline=True)
            embed.add_field(name='Đừng chửi ta', value='Vì Người cũng không muốn bị chửi, phải không nào... Nếu đã lỡ mồm thì phải xin lỗi nghe chưa?', inline=True)
            embed.add_field(name='Hãy góp ý', value='Vì ta không biết nhiều về thế giới hiện đại này lắm, nên là Người hãy chỉ cho ta biết là cần phải học thêm gì nhé. Ta sẽ cân nhắc nanoja.', inline=True)
        ...
        if message.content.lower().find('cưới ') == 0 and len(message.content) > 5:
            if client.get_user(int(userIdFilter(message.content))) == None:
                await message.channel.send('Tag lại đối tượng của mình đi, '+message.author.mention)
            elif userIdFilter(message.content) == '589315866282164229' and message.author.id != 462170598399803394:
                await message.channel.send('Cảm ơn tấm lòng của Người, nhưng tấm thân này đã thuộc về Rảnh mất rồi... Hãy tìm ai đó tốt hơn ta đi nanoja.')
            elif userIdFilter(message.content) == '589315866282164229' and message.author.id == 462170598399803394:
                await message.channel.send('N-Người nói gì vậy? Chúng ta đã thuộc về nhau rồi cơ mà, '+message.author.mention)
            else:
                marry = 1
                ...
                if marryPartner+' '+marryProposer in text or marryProposer+' '+marryPartner in text:
                    await message.channel.send('Ủa, hai người đã cưới nhau rồi mà? Đùa hoài nanoja!')
                else:
                    await message.channel.send(message.author.mention+' đã cầu hôn '+client.get_user(int(userIdFilter(message.content))).mention+'! Câu trả lời sẽ là gì đây? Ấn \'y\' để đồng ý hoặc ấn \'n\' để từ chối!')
        if message.content.lower() == 'y' and marry == 1 and marryPartner == str(message.author.id):
            marry = 0
            ...
            await message.channel.send('🎉🎉🎉 Xin chúc mừng '+client.get_user(int(marryProposer)).mention+' và '+message.author.mention+' đã đến được bên nhau nanoja!🎊🎊🎊 Chúc hai người hạnh phúc nanoja!🎉🎉🎉')
            marryPartner = ''
            marryProposer = ''
        if message.content.lower() == 'n' and marry == 1 and marryPartner == str(message.author.id):
            await message.channel.send('Ôi tiếc quá, '+client.get_user(int(marryProposer)).mention+' đã bị '+message.author.mention+' từ chối! Hãy kiên nhẫn chờ đúng người, đúng thời điểm nhé nanoja!')
            marry = 0
            marryPartner = ''
            marryProposer = ''
        ...
        if message.content.lower().find('ly dị ') == 0 and len(message.content) > 6:
            read = open("couples.txt","r",encoding="utf-8")
            text = read.read()
            read.close()
            # print(text)
            ...
            if client.get_user(int(userIdFilter(message.content))) == None:
                await message.channel.send('Tag lại đối tượng của mình đi, '+message.author.mention)
            elif userIdFilter(message.content)+' '+str(message.author.id) not in text and str(message.author.id)+' '+userIdFilter(message.content) not in text:
                await message.channel.send('Ủa, hai người có cưới nhau đâu mà đòi ly dị nanoja?')
            else:
                divorce = 1
                divorcePartner = userIdFilter(message.content)
                divorceProposer = str(message.author.id)
                await message.channel.send('Ôi không, '+message.author.mention+' muốn ly dị '+client.get_user(int(userIdFilter(message.content))).mention+'! Liệu đối phương có muốn ly dị không? Ấn \'y\' để đồng ý hoặc ấn \'n\' để từ chối!')
        
        if message.content.lower() == 'y' and divorce == 1 and divorcePartner == str(message.author.id):
            divorce = 0
            ...
            with open("couples.txt", "w") as f:
                for line in lines:
                    if line.find(divorcePartner+' '+divorceProposer)<0 and line.find(divorceProposer+' '+divorcePartner)<0:
                        f.write(line)
            
            await message.channel.send(client.get_user(int(divorceProposer)).mention+' đã ly dị '+message.author.mention+'! <:Aqua_cry:595633225637036043> <:Aqua_cry:595633225637036043> <:Aqua_cry:595633225637036043>')
            divorcePartner = ''
            divorceProposer = ''
        if message.content.lower() == 'n' and divorce == 1 and divorcePartner == str(message.author.id):
            await message.channel.send(message.author.mention+' vẫn còn muốn níu kéo '+client.get_user(int(divorceProposer)).mention+' ! Hai người nên đóng cửa bảo nhau nhé!')
            divorce = 0
            divorcePartner = ''
            divorceProposer = ''
        #Pixiv's url
        if message.content.lower().find('//www.pixiv.net/') >= 0:
            if message.content.lower().find('member_illust.php?illust_id=') > 0:
                id = message.content.split('illust_id=')[1].split('&')[0]
            elif message.content.lower().find('/artworks/') >= 0:
                id = message.content.split('/artworks/')[1]
            api = AppPixivAPI()
            api.login("unknowmember1990@gmail.com","0918273645")
            ...
            if(illust.page_count == 1):
                ...
                await message.channel.send("Album gồm "+ str(illust.page_count)+" ảnh nanoja!")
            ...
            # print(illust)
            if pixiv_image_rsp_fp.getbuffer().nbytes >= 8388608:
                await message.channel.send("Ảnh lớn hơn 8mb nên ta sẽ post ảnh resize nanoja!")
                if(illust.page_count == 1):
        ...
        # Twitter's url
        twitter_switch = 0
        if twitter_switch == 1 and message.content.lower().find('//twitter.com/') >= 0 :
            ...
            if hasattr(status, 'extended_entities'):
                if 'media' in status.extended_entities:
                    ...
                    for pic in status.extended_entities['media']:
                        if 'video' not in pic['media_url'] and count != 0:
        ...
        #Weather forecast
        if message.content.lower().find(".weather ") == 0 :
            ...
            if info["cod"] != "404":
                ...
                embed.add_field(name = 'Description', value = str(weather[0]['description']), inline=True)
                embed.add_field(name = 'Temprature', value = temp + "°C", inline=True)
                embed.add_field(name = 'Humidity', value = str(main["humidity"]) + "%", inline=True)
                embed.set_thumbnail(url=city_img_url)
                await message.channel.send(embed=embed)
                
            else:
                await message.channel.send("Không tìm thấy thành phố đó nanoja...")
        #Search for image
        ...
        if message.content.lower().find(".ott ") == 0:
            if message.content.lower().split(" ")[1] == "bao" or message.content.lower().split(" ")[1] == "búa" or message.content.lower().split(" ")[1] == "kéo":
                ...
                with open("jankenpon.txt", "r") as f:
                    ...
                    for line in lines:
                        if str(message.author.id) in line:
                            ...
                            lineOld = playerId + " " + str(win) + " " + str(tie) + " " + str(lose) + " " + str(score) + "\n"
                            if result == -1:
                                lose = lose + 1
                                score = score - 10
                            if result == 0:
                            ...
                            if result == 1:
                                win = win + 1
                                score = score + 10
                            lineNew = playerId + " " + str(win) + " " + str(tie) + " " + str(lose) + " " + str(score) + "\n"
                            # await message.channel.send(lines)
                lines[lines.index(lineOld)] = lineNew
                if score >= 1000:
                    await message.author.add_roles(message.author.guild.get_role(658325379538354187))
                elif score >= 500:
                    await message.author.add_roles(message.author.guild.get_role(658325189834178572))
                elif score >= 200:
                    await message.author.add_roles(message.author.guild.get_role(658325047282499614))
                elif score >= 100:
                    await message.author.add_roles(message.author.guild.get_role(658324756864565278))
                
                with open("jankenpon.txt", "w") as f:
                    f.writelines(lines)

                if result == 0:
                    await message.channel.send(senko_janken + " - Chúng ta hòa rồi!")
                elif result == -1:
                    await message.channel.send(senko_janken + " - Hoan hô, ta thắng rồi!")
                elif result == 1:
                    await message.channel.send(senko_janken + " - Hic, ta thua rồi <:Aqua_cry:595633225637036043>")
            else:
                await message.channel.send("Mời Người chọn bao, búa hoặc kéo.")
        if message.content.lower().find(".ottstatus") == 0:
            embed = discord.Embed(
                    title = "Thành tích oẳn tù tì của "+message.author.name,
                    colour = discord.Colour.orange()
                )
            with open("jankenpon.txt", "r") as f:
                lines = f.readlines()
